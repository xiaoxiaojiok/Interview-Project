...
- 地址解析
- 证书验证：发送https到服务端，服务端返回带公私钥的证书客户端校验
- 秘钥和加密算法协商：三个随机数生成秘钥
- 对称秘钥通讯：通过非对称来传输对称秘钥，再用对称来传递数据（效率高）
...
- Get /index HTTP/1.1
- HTTP/1.1 200 OK
- 200 301 302 400 401 403 404 500 502 503 504
...
- 客户端，HTTP 请求的头部，connection 设置为 keep-alive，保持存活一段时间：现在的浏览器，一般都这么进行了 2、服务器端，
...
1. http1.0和http1.1：长连接和管道传输（多个请求可以同时发送）
2. http1.1和http2.0：二进制，头部压缩，stream的并发传输，服务端主动推送
3. http2.0和http3.0：无队头阻塞(只影响某个stream,stream之间不影响)，更快速的连接（quic三次握手包括了tsl的握手信息），连接迁移(连接id上下文)
4. http2.0以上都是基于https的，http1.1队头阻塞在响应处理，http2.0队头阻塞在tcp层的stream报文
...
0. url解析（方法，路径，域名，协议等），构造http请求（请求行请求头请求体）
